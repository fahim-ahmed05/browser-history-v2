...
class BrowserHistory(Flox):
    ...
    def __init__(self):
        ...
        self.default_browser = self.settings.get('default_browser', 'All Browsers').lower()
        ...
        for browser_name in browser_names:
            ...
            if browser_name == CUSTOM_PROFILE:
                ...
                try:
                    if self.search_all_profiles:
                        profiles = browsers.get_all_profiles(browser_name, custom_profile_path=self.custom_profile_path)
                        if profiles: self.browsers.extend(profiles)
                    else:
                        b = browsers.get(browser_name, custom_profile_path=self.custom_profile_path, profile_last_updated=self.profile_last_updated)
                        if b: self.browsers.append(b)
            ...
            if self.search_all_profiles:
                self.browsers.extend(browsers.get_all_profiles(browser_name))
            else:
                b = browsers.get(browser_name, profile_last_updated=self.profile_last_updated)
                if b:
        ...
        if self.all_browsers_history and self.custom_profile_path:
            try:
                if self.search_all_profiles:
                    b_custom_list = browsers.get_all_profiles(CUSTOM_PROFILE, custom_profile_path=self.custom_profile_path)
                    self.browsers.extend(b_custom_list)
                else:
                    b_custom = browsers.get(CUSTOM_PROFILE, custom_profile_path=self.custom_profile_path, profile_last_updated=self.profile_last_updated)
                    if b_custom: self.browsers.append(b_custom)
            except Exception:
        ...
        if not self.browsers and not self.init_error:
            self.init_error = "Could not locate any supported browser profile databases."
